...
try {
    ...
    if ($pickup['total'] <= 0) {
        ...
        echo json_encode(['status' => 'error', 'message' => 'এই ছাত্রটি পিকআপ হয়নি।']);
        exit();
    }

    // পিকআপ তথ্য সেভ করুন
    $stmt = $pdo->prepare("UPDATE pickup_and_drop SET drop_time = NOW() WHERE student_id = :student_id AND route_attendant_id = :route_attendant_id AND DATE(pickup_time) = CURDATE() AND drop_time IS NULL");
    $stmt->execute(['student_id' => $student["id"], 'route_attendant_id' => $route_attendant_id]);
    echo json_encode(['status' => 'success', 'message' => 'ছাত্র নামানো সফল হয়েছে।']);
