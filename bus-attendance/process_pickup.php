...
$student_id = (int)trim($query_params['id'] ?? '');
$route_id = (int)trim($query_params['route_id'] ?? '');
...
// echo json_encode(['status' => 'success', 'message' => 'QR কোড সঠিকভাবে স্ক্যান করা হয়েছে।', 'student_id' => $student_id, 'route_id' => $route_id]);
try {
    ...
    //route attendant route check
    $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM route_attendant WHERE id = :id AND DATE(created_at) = CURDATE() AND route = :route_id LIMIT 1");
    $stmt->execute(['id' => $route_attendant_id, 'route_id' => $route_id]);
    $attendant = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$attendant || $attendant['total'] == 0) {
        http_response_code(403);
        echo json_encode(['status' => 'error', 'message' => 'আপনার রুটে এই ছাত্রের পিকআপ করার অনুমতি নেই।']);
        exit();
    }
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM pickup_and_drop WHERE student_id = :student_id AND route_attendant_id = :route_attendant_id AND DATE(pickup_time) = CURDATE()");
    $stmt->execute(['student_id' => $student_id, 'route_attendant_id' => $route_attendant_id]);
